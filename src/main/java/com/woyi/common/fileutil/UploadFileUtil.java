...
public abstract class UploadFileUtil {
	 ...
	 */
	private static String[] filePostfixs = { "*" };
	/**
	 * 定义可以上传文件的后缀数组
	 */
	private static String[] typeImages = { "gif", "jpeg", "png", "jpg", "tif", "bmp" };
	/**
	 * 定义可以上传文件的后缀数组
	 */
	private static String[] typeOthers = { "html", "htm", "doc", "xls", "txt", "zip", "rar", "pdf",
			"cll" };
	/**
	 * 上传文件的最大长度(2G)
	 */
	 ...
	 * @throws Exception
	 */
	public static UploadState upload4Stream(String fileName, String path, InputStream is)
			throws Exception {
		init();
	 ...
	 * @throws Exception
	 */
	public static UploadState upload4Stream(String fileName, String path, File file)
			throws Exception {
		init();
		UploadState state = UploadState.UPLOAD_FAILURE;
	 ...
	 * @throws Exception
	 */
	public static UploadState upload4Stream(String fileName, String path, InputStream fs,
			String[] allowTypes) throws Exception {
		UploadState state = UploadState.UPLOAD_FAILURE;
		if (validTypeByName(fileName, allowTypes)) {
	 ...
	 * @throws Exception
	 */
	public static boolean upload4CopyFile(String fileName, String path, File file)
			throws Exception {
		init();
	...
	/**
	 * 验证当前后缀、文件类型是否是图片类型 typeImages 可以设置图片类型
	 * @since Oct 10, 2010 12:17:18 PM
	...
	/**
	 * 验证当前后缀、文件类型是否是非图片类型（常用办公文件类型） typeOthers 可以设置文件类型
	 * @since Oct 10, 2010 12:18:18 PM
	 * @param suffix 验证文件的后缀
	 * @return 是否合法
	 */
	...
	/**
	 * 验证当前文件名、文件类型是否是图片类型 typeImages 可以设置图片类型
	 * @since Oct 10, 2010 12:19:18 PM
	...
	/**
	 * 验证当前文件名称、文件类型是否是非图片类型（常用办公文件类型） typeOthers 可以设置文件类型
	 * @since Oct 10, 2010 12:21:22 PM
	 * @param fileName 验证文件的名称
	 * @return 是否合法
	 */
	...
	/**
	 * 删除某个文件夹下的所有文件（除目录），包含子文件夹的文件
	 * @since Oct 12, 2010 9:30:01 PM
	...
	/**
	 * 递归处理文件名称，直到名称不重复（对文件名、目录文件夹都可用） eg: a.txt --> a(1).txt 文件夹upload--> 文件夹upload(1)
	 * @since Oct 10, 2010 12:56:27 AM
	 * @param fileName 文件名称
	 * @param path 文件路径
	 * @param num 累加数字，种子
	 * @return 返回没有重复的名称
	 */
	public static String getBracketFileName(String fileName, String bracketName, String path,
			int num) {
	 ...
	 * Image src = javax.imageio.ImageIO.read(file); // 构造Image对象 int old_w = src.getWidth(null); //
	 * 得到源图宽 int old_h = src.getHeight(null); int new_w = 0; int new_h = 0; // 得到源图长 float
	 * tempdouble; if (old_w >= old_h) { tempdouble = old_w / width; } else { tempdouble = old_h /
	 * height; } if (old_w >= width || old_h >= height) { // 如果文件小于锁略图的尺寸则复制即可 new_w =
	 * Math.round(old_w / tempdouble); new_h = Math.round(old_h / tempdouble);// 计算新图长宽 while (new_w
	 * > width && new_h > height) { if (new_w > width) { tempdouble = new_w / width; new_w =
	 * Math.round(new_w / tempdouble); new_h = Math.round(new_h / tempdouble); } if (new_h > height)
	 * { tempdouble = new_h / height; new_w = Math.round(new_w / tempdouble); new_h =
	 * Math.round(new_h / tempdouble); } } BufferedImage tag = new BufferedImage(new_w, new_h,
	 * BufferedImage.TYPE_INT_RGB); tag.getGraphics().drawImage(src, 0, 0, new_w, new_h, null); //
	 * 绘制缩小后的图 FileOutputStream newimage = new FileOutputStream(file); // 输出到文件流 JPEGImageEncoder
	 * encoder = JPEGCodec.createJPEGEncoder(newimage); JPEGEncodeParam param =
	 * encoder.getDefaultJPEGEncodeParam(tag); param.setQuality((float) (100 / 100.0), true);//
	 * 设置图片质量,100最大,默认70 encoder.encode(tag, param); encoder.encode(tag); // 将JPEG编码
	 * newimage.close(); } }
	 */
